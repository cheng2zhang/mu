...
def usage():
  ...
  OPTIONS:
   ...
   -x, --nxi:   number of points for scanning the charging parameter
   -X, --xi:    type of the charging parameter, f, r, or t
...
def doargs():
  ...
  global rhomin, rhomax, drho, temp, pottype
  global nxi, xitype, cmd0, fnout, fnscan, verbose
...
def rho_scan():
  ...
  while 1:
    ...
    cmd = cmd0 + (
          " -T %s --pot=%s -x %s -X %s -r %s -O %s -C %s -s %s --mdamp=%s --nbases=%s --tol=%s" % (
          temp, pottype, nxi, xitype, rho, fnout,
          ietype, ies, mdamp, nbases, tol) )

...
def hsexact():
  ...
  for i in range(1, 10000):
    den = 0.01*i
    if den > 1.000001: break
    rho = den
    bP = getP(vir, rho)
    bmu = getmu(vir, rho)
    print "%.2f %s %s %s" % (den, bP, bP/rho - 1, bmu)
...
def doit(fn, col, n1 = -1, n2 = -1):
  vir = getvir(fn, col)
  normalize(vir)
  text  = "# generated by running \"%s\"\n" % (" ".join(sys.argv))
  text += "#  rho  beta*P/rho-1     beta*P            beta*mu\n"
  i = 1
  while 1:
    rho = rhodel * i
    if rho > 1.0 + 0.5 * rhodel:
      break
    ...
    if n1 > 0 and n2 > 0:
      ...
      pres = getP(vir, rho)
      mu = getmu(vir, rho)
    line = "%-6s %-16s %-16s %-16s" % (rho, pres/rho - 1, pres, mu)
    text += line.rstrip() + "\n"
...
if __name__ == "__main__":
  if len(sys.argv) > 1:
    ...
    # we use the second command-line argument
    # to determine the column ID 
    # The column ID is zero based, and 0 means the order 
    if len(sys.argv) > 2:
      col = int(sys.argv[2])
    else:
      col = 1
    ...
    # at n1 and n2, and use their averages
    if len(sys.argv) >= 5:
      n1 = int(sys.argv[3])
      n2 = int(sys.argv[4])
    else:
